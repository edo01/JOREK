 ...
 * the Fortran storage(AoS), layout_left(SoA) over packed buffers (jgx/pack.h) -- and a
 ...
 *
 * A record <name> provides an enum <name>_field (the field ids), a
 * <name>_soa_ptrs, and one struct deriving jgx::record_set that carries the
 * field views and, as static members, everything that builds it:
 *
 *   <name>_set<L, Real, Int>::from_aos(base, desc, n)  over the Fortran
 *                                                      storage, always AoS
 *                             from_soa(ptrs, desc, n)  over packed buffers
 *                             record()                 the checked descriptor
 *                             from_registry(base, n)   record() + from_aos
 *
 ...
 * is no wrong-layout call to get wrong. record() and from_registry are
 * host-only, the registry being host storage; the makers are JGX_HD.
 *
 ...
 *
 * The two makers build:
 *
 *   aos  `base` is the Fortran array itself. A component sits at its offset
 *        inside a record and repeats at the record stride, so nothing is copied
 *        and nothing is owned -- the set is valid as long as that array is.
 *   soa  `ptrs` are one packed buffer per field, owned by a
 *        jgx::host_pack_buffers or a jgx::device_pack that must outlive the
 *        set. Offsets and the record stride play no part here; only the kind
 *        and the intra extents do.
 */
...
/* One packed buffer per field, indexed by the record's field enum. Filled by
 * whoever owns the pack; */

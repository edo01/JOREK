/* node_variants/fullmhd/node_variant_set.h -- the node set of a full-MHD model.
 *
...
#include <cstddef>
#include "datatypes/data_structure/node_variants/fullmhd/node_fullmhd_set.h"

namespace jorek {

template <class L, class Real = double, class Int = int>
using node_model_set = node_fullmhd_set<L, Real, Int>;
...
inline node_model_set_aos node_model_set_from_registry(void* base,
                                                       std::size_t n_nodes) {
  return node_fullmhd_set_from_registry(base, n_nodes);
}
